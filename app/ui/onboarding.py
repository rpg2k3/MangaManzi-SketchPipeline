"""First-run API key onboarding wizard — OpenAI + Anthropic, fail-soft validation."""
...
class OnboardingDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setModal(True)
        self._statuses = {"openai": "untested", "anthropic": "untested"}
        self._build_ui()
        self._load_existing()
    ...
    def _build_ui(self):
        ...
        header = QLabel("Welcome to 9LivesK9 — Chat_GPT Pipeline")
        header.setFont(QFont("", 16, QFont.Weight.Bold))
        header.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(header)

        sub = QLabel(
            "Enter your API keys. Both OpenAI and Anthropic are required.\n"
            "Testing is optional — rate-limit warnings can be safely ignored."
        )
        ...
        lane_label = QLabel(
            "Pipeline:  Claude (sonnet-4) engineers prompts → "
            "OpenAI gpt-image-1 renders images."
        )
        ...
        # --- OpenAI ---
        og = QGroupBox("OpenAI (gpt-image-1 — image generation)")
        of = QFormLayout(og)
        ...
        # --- Anthropic ---
        ag = QGroupBox("Anthropic (Claude — prompt engineer)")
        af = QFormLayout(ag)
    ...
    def _test_openai(self):
        self._test_provider("openai", self.openai_edit, key_tests.test_openai)

    def _test_anthropic(self):
        self._test_provider("anthropic", self.anthropic_edit, key_tests.test_anthropic)

    def _on_continue(self):
        has_failure = any(s not in ("ok", "untested") for s in self._statuses.values())
        if has_failure:
            ...
            reply = QMessageBox.question(
                ...
                "production calls. Proceed anyway?",
